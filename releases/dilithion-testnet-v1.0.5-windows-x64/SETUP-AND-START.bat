@echo off
REM =======================================================
REM  DILITHION TESTNET - INTERACTIVE SETUP WIZARD V2.1
REM =======================================================
REM  Fixed version with improved disk space detection
REM =======================================================
...
set "FREE_GB=unknown"
for /f "skip=1 tokens=2" %%a in ('wmic logicaldisk where "DeviceID='%CD:~0,2%'" get FreeSpace') do (
    set FREE_BYTES=%%a
    goto :got_free_bytes
)
:got_free_bytes
...
REM If we got bytes, convert to GB with safe error handling
if not "%FREE_BYTES%"=="" (
    if not "%FREE_BYTES%"=="unknown" (
        REM Remove any commas and spaces from the number
        set FREE_BYTES=%FREE_BYTES:,=%
        set FREE_BYTES=%FREE_BYTES: =%
        REM Calculate GB using arithmetic division (safer than string manipulation)
...
REM Verify FREE_GB is actually numeric and non-empty before comparison
if "%FREE_GB%"=="" (
    echo    WARNING: Could not detect disk space (continuing anyway)
    goto :skip_disk_check
)

REM Safe numeric comparison with error suppression
...
if %errorlevel% equ 0 (
    ...
    echo  ========================================================
    echo.
    echo  Another instance is already running on this system.
    echo.
    echo  Please close it first using one of these methods:
    echo    - Press Ctrl+C in the other window
    echo    - Close the other command window
    echo    - Use Task Manager to end dilithion-node.exe
    echo.
...
REM =======================================================
REM If threads is "auto", omit the --threads parameter (node will auto-detect)
if "%threads%"=="auto" (
    dilithion-node.exe --testnet --addnode=170.64.203.134:18444 --mine
) else (
