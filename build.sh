#!/bin/bash
# Phase 9.3: OSS-Fuzz build script
# This script builds fuzz targets for Google OSS-Fuzz
...
# Build additional fuzz targets if needed
# make fuzz_compactsize && cp fuzz_compactsize $OUT/
# make fuzz_network_message && cp fuzz_network_message $OUT/
# make fuzz_address && cp fuzz_address $OUT/
# make fuzz_difficulty && cp fuzz_difficulty $OUT/
# make fuzz_subsidy && cp fuzz_subsidy $OUT/
# make fuzz_merkle && cp fuzz_merkle $OUT/
# make fuzz_tx_validation && cp fuzz_tx_validation $OUT/
# make fuzz_utxo && cp fuzz_utxo $OUT/
