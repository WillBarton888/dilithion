...
BOOST_AUTO_TEST_CASE(test_ibd_coordinator_integration) {
    ...
    BOOST_CHECK_EQUAL(headers_manager.GetBestHeight(), 0);  // No headers yet
    BOOST_CHECK_EQUAL(block_fetcher.GetBlocksInFlight(), 0);  // No blocks in flight
    BOOST_CHECK_EQUAL(peer_manager.GetConnectionCount(), 0);  // No peers
    
    // Tick should do nothing when synced
    coordinator.Tick();
    
    BOOST_CHECK_EQUAL(block_fetcher.GetBlocksInFlight(), 0);  // Still no blocks
}
...
BOOST_AUTO_TEST_CASE(test_ban_threshold_logic) {
    // Test that peers are banned when exceeding threshold
    CPeerManager peer_manager("");
    ...
    if (peer) {
        ...
        for (int i = 0; i < ban_threshold; i += 10) {
            bool should_ban = peer_manager.Misbehaving(peer_id, 10);
            
            // Should ban when threshold is reached
            if (i + 10 >= ban_threshold) {
                BOOST_CHECK(should_ban);
            }
        }
