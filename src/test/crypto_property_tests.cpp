...
{
    // Generate key pair
    uint8_t public_key[DILITHIUM3_PUBLICKEYBYTES];
    uint8_t private_key[DILITHIUM3_SECRETKEYBYTES];
    ...
    // Sign message
    uint8_t valid_signature[DILITHIUM3_BYTES];
    dilithium3_sign(valid_signature, nullptr, 
                    message.data(), message.size(), 
                    private_key);
    ...
    uint8_t modified_signature[DILITHIUM3_BYTES];
    std::memcpy(modified_signature, valid_signature, DILITHIUM3_BYTES);
    modified_signature[0] ^= 0xFF; // Flip bits
    
    int verify_result = dilithium3_verify(modified_signature, DILITHIUM3_BYTES,
                                          message.data(), message.size(),
                                          public_key);
    BOOST_REQUIRE_MESSAGE(verify_result != 0,
